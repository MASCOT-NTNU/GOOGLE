"""
This script builds the RRTHome
Author: Yaolin Ge
...
class RRTStarHome:
    ...
    def search_path_from_trees(self, x_current, y_current, x_target, y_target):
        t1 = time.time()
    ...
    def is_location_legal(self, x, y):
        point = Point(x, y)
        islegal = True
        if self.polygon_obstacle_shapely.contains(point):
            islegal = False
        return islegal
    ...
    def is_path_legal(self, x1, y1, x2, y2):
        ...
        if (self.line_border_shapely.intersects(line) or
                self.line_obstacle_shapely.intersects(line)):
            islegal = False
        return islegal
    ...
    def check(self):
        ...
        xn = 3000
        yn = -300
        # cv.budget.budget_left = 4900
        # cv.update_cost_valley(self.mu, Sigma, xn, yn, xp, yp)
        # cv.get_cost_valley()
        plt.figure()

        from GOOGLE.Simulation_2DNidelva.Config.Config import X_HOME, Y_HOME
        self.search_path_from_trees(xn, yn, X_HOME, Y_HOME)
