...
class GOOGLE2Launcher:
    ...
    def run(self):
        ...
        self.result_pathplanner = self.pool.apply_async(self.rrtstar.search_path_from_trees, args=(self.CV.cost_valley,
                    ...
                    if (self.auv.auv_handler.getState() == "waiting" and
                        ...
                        if self.gohome:
                            if np.sqrt((X_HOME - self.x_current) ** 2 + (Y_HOME - self.y_current) ** 2) <= TARGET_RADIUS:
                                ...
                                break
                        self.x_next, self.y_next = self.result_pathplanner.get()
                        self.x_previous = self.x_current
                        self.y_previous = self.y_current
                        ...
                        if not self.gohome:
                            self.result_pathplanner = self.pool.apply_async(self.rrtstar.search_path_from_trees,
                            ...
                            if self.obstacle_in_the_way:
                                self.result_pathplanner = self.pool.apply_async(self.rrthome.search_path_from_trees,
                                                                                args=(self.x_next,
                                                                                     self.y_next,
                                                                                     X_HOME,
                                                                                     Y_HOME))
                                ...
                                self.result_pathplanner = self.pool.apply_async(self.straight_line_planner.get_waypoint_from_straight_line,
