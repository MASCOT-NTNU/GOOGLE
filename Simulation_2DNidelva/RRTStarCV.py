...
class RRTStarCV:
    ...
    def search_path_from_trees(self, cost_valley, polygon_budget_ellipse, line_budget_ellipse, x_current, y_current):
        ...
        for i in range(1, MAX_ITERATION-1):
            ...
            dx1 = (self.tree_table[ind_nearest, 0] - self.tree_table[i, 0]) ** 2
            dy1 = (self.tree_table[ind_nearest, 1] - self.tree_table[i, 1]) ** 2
            dd1 = np.sqrt(dx1 + dy1)
            if dd1 > STEPSIZE:
                angle = np.math.atan2(self.tree_table[i, 0] - self.tree_table[ind_nearest, 0],
                                      self.tree_table[i, 1] - self.tree_table[ind_nearest, 1])
                self.tree_table[i, 1] = self.tree_table[ind_nearest, 1] + STEPSIZE * np.cos(angle)
                self.tree_table[i, 0] = self.tree_table[ind_nearest, 0] + STEPSIZE * np.sin(angle)
            ...
            for ind_neighbour in ind_neighbours:
                x1 = self.tree_table[i, 0]
                y1 = self.tree_table[i, 1]
                x2 = self.tree_table[ind_neighbour, 0]
                y2 = self.tree_table[ind_neighbour, 1]
                cost_path1 = self.get_cost_along_path(x1, y1, x2, y2, self.tree_table[i, 2])
        ...
        path_to_target = []
        path_to_target.append([x_target, y_target])
        if self.tree_table[-1, 3] != FAIL_TO_GENERATE_PATH:
            ...
            while self.tree_table[ind_pointer, 3] != 0:
                ...
                path_to_target.append([self.tree_table[ind, 0], self.tree_table[ind, 1]])
                ind_pointer = ind
        else:
            path_to_target.append([x_current, y_current])
        path_to_target = np.flipud(np.array(path_to_target))

        dx4 = (path_to_target[1, 0] - x_current)**2
        dy4 = (path_to_target[1, 1] - y_current)**2
        dd4 = np.sqrt(dx4 + dy4)

        if dd4 > STEPSIZE:
            angle = np.math.atan2(path_to_target[1, 0] - x_current,
                                  path_to_target[1, 1] - y_current)
            self.y_next = y_current + STEPSIZE * np.cos(angle)
            self.x_next = x_current + STEPSIZE * np.sin(angle)
        else:
            self.x_next = path_to_target[1, 0]
            self.y_next = path_to_target[1, 1]
        ...
        #
        # plt.plot(path_to_target[:, 1], path_to_target[:, 0], 'r')
