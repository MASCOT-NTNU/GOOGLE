...
class TestDirection(TestCase):
    ...
    def test_get_direction_field(self):
        ...
        plt.scatter(self.grid[:, 0], self.grid[:, 1], c=df, cmap="RdBu", vmin=0, vmax=1, alpha=.4)
        xn, yn = self.d.get_current_location()
        plt.plot([xp, xn], [yp, yn], 'k-', linewidth=2)
        plt.plot(xn, yn, 'r.', markersize=20, label="Current location")
        plt.plot(xp, yp, 'b.', markersize=20, label="Previous location")
        plt.colorbar()
        plt.legend()
        plt.show()
        ...
        plt.scatter(self.grid[:, 0], self.grid[:, 1], c=df, s=1000, cmap="RdBu", vmin=0, vmax=1, alpha=.4)
